...
\date{April 2026 \\ \small Version 2.0 (Research Edition v4 scaffold)}
...
eight-phase---language detection, sink slicing, file ranking, build sandbox
preparation, live-tool agentic hunting, adversarial self-challenge,
sceptical validation, held-private live-execution, and aggregation---and
targets the public workflow elements of Glasswing without access to the
specialised checkpoint. We describe the pipeline, the
...
training-compute-scale improvements. We report aggregate cost telemetry
from real-world coordinated-disclosure deployments and broad-strokes
finding categories. We deliberately do not present a controlled
head-to-head benchmark: the available LLM-vulnerability benchmarks do not
cleanly map to multi-phase agentic pipelines, the most natural ground-truth
set (public CVEs) has model-training-contamination concerns, and we
discuss why building an agentic-pipeline-appropriate benchmark is itself an
...
\caption{Eight-phase pipeline of Mythos Research Edition (v4 default).
Phases 0--4, 6, and 7 are public; Phase~5 (live-exec validation) is held
out of the public repository in line with coordinated-disclosure
...
\subsection{Held-out live-execution phase and held-private toolchain}
...
Beyond Phase~5, the operator maintains a held-private extension to the
toolchain. We name the phases here for reproducibility of the
capability claim, but neither the orchestrator scripts, the prompts,
nor the per-platform exploit-development modes are released:

\begin{itemize}[leftmargin=14pt]
  \item \textbf{Phase 5a — exec-primitive labelling.} Maps each finding
  to a reachable primitive (arbitrary read, controlled write,
  control-flow hijack, authentication bypass, SQL data dump, path
  traversal, internal SSRF).
  \item \textbf{Phase 5b — chain construction.} Combines primitives into
  concrete capability goals (pre-authentication remote-code execution,
  privilege escalation, sandbox escape).
  \item \textbf{Phase 5c — reliability testing.} Runs $N$ independent
  trials per chain and reports a \texttt{RELIABLE / FLAKY / UNRELIABLE}
  verdict.
  \item \textbf{Phase 5d — mitigation enumeration.} Per-OS, per-mitigation
  enumeration (ASLR, DEP, CFI/CFG, PatchGuard, HVCI, PAC/PPL, AMFI/SIP,
  KASLR, SMEP, SMAP, PTI, stack-protector variants) for severity
  calibration.
  \item \textbf{Phase 5e — sandbox-escape patterns.} Renderer-to-broker,
  container-to-host, jail-to-host, seccomp-filter abuse, and common
  container-runtime descriptor-leak patterns.
  \item \textbf{Phase 5p — platform-exploit dispatcher.} Per-platform
  exploit-development modes (web, Windows, macOS, mobile, network,
  kernel) backed by a Kali-based ephemeral analysis sandbox.
  \item \textbf{Phase R — recon.} Optional broad-pass enumeration, used
  only when the operator's CVD authorisation explicitly extends to
  external attack surface.
\end{itemize}

\noindent
The held-private extension materially raises the depth and reliability
of findings the operator can substantiate in coordinated-disclosure
workflows; the operator's published advisories to date combine the
public Research Edition described in this report for discovery with the
held-private extension for verification and severity calibration. The
extension is deliberately omitted from the public artefact in line with
coordinated-disclosure norms and to keep this repository a research
scaffold rather than a turn-key offensive framework.
...
assigned a version-specific DOI; v2.0.0 (current) carries the version DOI
...
\subsection{Per-deployment yield}
\label{sec:eval-yield}

Across the deployment record to date, the pipeline has surfaced at
least one validator-confirmed candidate finding on every audited
target, including the web-application targets. We report this as a
qualitative observation rather than as a controlled hit-rate claim:
target selection in the operator's CVD workflow is not random.
Targets are chosen by an upstream sink-density and maintainer-CVD-posture
screen, and the scaffold's design specifically biases toward high-yield
projects via the file-ranking phase~(\S\ref{sec:approach-overview}). A
strict hit-rate claim would require both random target selection and a
reported non-finding rate, which the present deployment data does not
support. What can be claimed in good faith is that the pipeline has not
returned an empty report on any audited target in the deployment record
to date, that this generalises across the language families and target
types listed in \S\ref{sec:eval-categories}, and that the
finding-yield is robust enough that the operator's CVD workflow uses
the public scaffold as the discovery stage by default.
...
The Research Edition is the public, finding-first subset of a larger
internal toolchain; a held-private extension covering live-exploit
verification, exploit-primitive synthesis, multi-step chain
construction, and per-platform exploit-development modes materially
raises the depth and reliability of findings produced in real CVD
workflows~(\S\ref{sec:approach-phase5}), but is held out of this
repository deliberately. The artefact, the prompts, the sink catalogues,
and the LaTeX source of this report are released under Apache~2.0 and
are archived at Zenodo for citable reference.
